...
# 网页门户要登录（账号密码 + 签名会话 cookie；MCP 那条线用地址令牌，不受影响）
bash lighthouse.sh kb passwd <id> --admin                          # 给自己设管理员密码（只打印一次）
bash lighthouse.sh kb passwd <id> --user zhangsan --person 张三     # 给同事开通网页账号
bash lighthouse.sh kb accounts <id>                                # 列出网页账号
...
- **两种拿法**：AI 里直接问能读到正文；也可以让 AI 给一条**限时下载链接**，
  或者同事用浏览器打开 `<他的地址>/files` 自己下载原件 / 抽取文本 / 勾选打包成 zip。
- **网页直接收资料**：管理页可以拖拽文件/整个文件夹上传（自动抽取文本、自动入库），
  再逐篇或**批量**定等级公开；每行等级可当场改，已公开的可一键下架。
- **手机/浏览器管理**：`…/<窗口>/admin?k=<管理令>` 一页管完 —— 扫描入库、逐篇定级公开、改等级、
  下架、看原件、批申请、发地址、发放/停用/换地址、用量看板；每步都留痕（actor=admin）。
