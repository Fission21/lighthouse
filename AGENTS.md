...
改了门户页面（kb 模式的网页）之后，接口测试过了不算完 —— 还要用真浏览器走一遍并看截图：

```bash
python tests/browser_check.py --window <窗口名> \
  --admin-url "http://127.0.0.1:<端口>/<窗路径>/admin?k=<管理令>" \
  --files-url "http://127.0.0.1:<端口>/<窗路径>/files?t=<同事令牌>" --shots /tmp/shots
```

它用 Chrome DevTools Protocol 发真鼠标事件（点、展开、拖放）走完管理页与同事页，每步截图，
最后按三条口径报告：**操作能不能走通 / 排版有没有挤在一起或被截断 / 手机宽度会不会左右晃**。
前置：本机 Chrome 带 `--remote-debugging-port=9222` 跑着。脚本会自己收尾（把挪动的资料放回原位、
删掉临时文件夹），可以直接对着真库跑。
