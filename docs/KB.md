...
> 界面约定（三条硬规矩）：
> ① **一件事一页**：管理页拆成 资料 / 人员 / 邀请码 / 用量 四个页签，每页只留一个主按钮，
>    其余动作降级或收进「更多设置 / 更多操作 / ⋯」；POST 完回到自己那一页；
> ② **页面上不留解释性小字**：说明一律挂到 `title` 或 `?` 悬停提示，页面上只留数据、状态回执、空状态一句；
> ③ **长列表默认收起 + 分页**（同事列表收成一行，点「编辑 ▾」展开；资料每页 20 篇、同事每页 8 位，
>    翻页链接带管理令与筛选条件）。加东西时别把大段说明直接铺在页面上。
>
> **给用户看/复制的链接一律是完整地址**（`_copybtn()` 负责复制，`_Portal._abs()` 负责补域名）。
> 页面里只写相对路径（`/w-xxx/register?c=…`）会让「复制」复制出半截链接 —— 踩过这个坑。


`<域名>/<窗口路径>/admin?k=<管理令>`（取法：`bash lighthouse.sh kb admin-url bidkb`）。维护分四页：

| 页签 | 路径 | 管什么 |
|---|---|---|
| 资料 | `/admin` | 上传、文件夹、资料清单 |
| 人员 | `/admin/people` | 待批申请、已授权同事、直接发地址 |
| 邀请码 | `/admin/invites` | 生成 / 停用 / 删除邀请码 |
| 用量 | `/admin/usage` | 按人 / 按天 / 按资料 / 按工具 |

| 区域 | 能干什么 |
|---|---|
| **资料 · 添加** | **把文件或整个文件夹拖进虚线框**（最省事，松手就自动上传）；或「选择文件」/「选择文件夹」。选好分类（或新建分类名）与等级，点「上传，先进待批」或「上传并直接公开」。文件夹层级原样保留，上传后自动抽取文本、自动入库；上传中有百分比进度 |
| **资料 · 清单** | 搜索 + 按状态/分类/等级筛选；**每行「等级」+「文件夹」列改完点「保存」**（一次同时生效，`@apply`），要放开就点「公开」；改标题/进回收站收在行内「⋯」里；「看原件」是你自己预览（不受等级限制，不占同事地址） |
...
## 四·六·五、文件夹管理与回收站（2026-09-24）

资料库里的**文件夹就是磁盘上的真实子目录**（不是台账里的虚拟字段），所以：
上传整个文件夹后层级原样保留；新建/移动/删除都在磁盘上做，台账的 `path` 与 `doc_id`
跟着改（文案/状态/等级/审批记录都保留）。

| 想干什么 | 在管理页怎么做 | 命令行同款 |
|---|---|---|
| 新建文件夹 | 「文件夹」区填名字 → 新建（建完自动进去） | `kb mkdir <窗口> <路径>` |
| 把资料挪进去 | **拖**资料行到文件夹行上放下；手机/触屏用行内「文件夹」下拉 + 「移动」；多选 → 「移到 <文件夹>」 | `kb mv <窗口> <doc_id> --to <文件夹>` |
| 给文件夹定默认等级 | 文件夹行选等级 → 「存默认等级」（新进来的文件继承它） | `kb flevel <窗口> <文件夹> <等级>` |
| 删文件夹 | 文件夹行「删除（进回收站）」→ 整棵进 `.回收站/` | `kb trash <窗口>` / `kb rmdir` |
| 误删了想拿回来 | 「回收站」区 → 「放回原位」（回到进回收站之前的状态） | `kb trash <窗口> --restore <名字>` |
| 彻底删 / 清 30 天前的 | 回收站区「彻底删」/「清理 30 天前的」（另有每周一的自动清理） | `kb trash <窗口> --purge <名字>` / `--purge-older 30` |
| 改标题（不动磁盘名） | 资料行标题框直接改 → 「改标题」 | `kb title <窗口> <doc_id> <新标题>` |
| 让 AI 给归类建议 | —（不给外部 AI 写权限） | `kb suggest <窗口>` → 出方案（按**任务需求**聚类，不按文件类型）；`kb apply <窗口> <方案.json> --dry-run` 先演练，去掉 `--dry-run` 才执行 |

**等级顺序**：上传时显式选的等级 > 所在文件夹（含父文件夹）默认等级 > 窗口默认等级；只影响新进来的文件。

**同事侧**：资料页按文件夹分组（可折叠），每个文件夹一个「打包下载这个文件夹」（只装他等级内的），
也可以勾选几篇一起打包。**AI 侧**：`kb_list(folder="技术")`、`kb_search(kw, folder="技术")`，
返回里带 `folder` 字段。

**边界**：`../`、`.`、绝对路径、`~`、盘符、隐藏名、超 6 层、怪字符**一律拒绝**（不做悄悄改写 ——
