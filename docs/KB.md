...
**两个入口是分开的**（2026-09-24）：同事走普通登录页，管理员走管理入口，互相拦并指路。

| 谁 | 入口 | 说明 |
|---|---|---|
| 同事 | `<窗口路径>/login` | 有「凭邀请码注册」入口、有找回密码说明 |
| 管理员 | `<窗口路径>/admin/login` | 只认管理员账号；不放注册/申请入口；跟管理页同一套网络准入（公网开关 / 部署机限制） |

- 两个入口**共用同一套账号与会话**（一个 cookie），分开只是让「该走哪个门」一眼看清。
- **走错门会被拦下**：管理员账号在同事页登录 → 401 + 指到管理入口；同事账号在管理入口登录 → 401 + 指到普通登录。**都不签发会话**（密码对也不行），并进审计（`reason=走错入口`）。
- 未登录打开 `/admin`、`/admin/usage` → 引导到管理入口；打开 `/files`、`/request` → 引导到普通登录。
- 带凭据（管理令 `?k=`、地址令牌、签名下载链接）依然免登录；`/login`、`/admin/login`、`/logout`、`/register` 本身免登录。
- **登录成功 = 302 直接跳到目标页**（管理入口 → 管理页，同事 → 资料页），cookie 就在 302 响应上；
  刷新不会重复提交表单（POST/Redirect/GET）。已登录的人再打开登录页也会被直接送走，不再给表单。
